...
export async function mountAdminPage() {
  ...
  if (_authSub?.subscription?.unsubscribe) {
    _authSub.subscription.unsubscribe();
    _authSub = null;
  }
  ...
  _authSub = supabase.auth.onAuthStateChange(async (_event, session) => {
...
async function _render() {
  ...
  if (!state.session) {
    ...
    el.innerHTML = _loginHtml(state.authError);
    _wireLogin();
    return;
  }
...
async function _checkAdmin() {
  const { data, error } = await getSupabaseClient().rpc('is_syncpad_admin');
  if (error) {
    state.authError = error.message || 'Failed to check admin access.';
    return false;
  }
  return !!data;
}
