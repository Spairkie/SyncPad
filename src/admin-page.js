...
export async function mountAdminPage() {
  ...
  if (error) state.authError = error.message || 'Could not check current auth session.';
  state.loading = false;

  _authSub = supabase.auth.onAuthStateChange((_event, session) => {
...
async function _render() {
  ...
  if (!state.session || state.pendingSignOut) {
    state.pendingSignOut = false;
    state.isAdmin = false;
    state.rooms = [];
  ...
  if (!adminCheck.ok) {
    ...
    el.innerHTML = _errorHtml('Could not verify admin access', state.adminCheckError);
    _wireErrorActions();
    return;
  }
...
async function _checkAdminWithTimeout(timeoutMs) {
  try {
...
async function _signOut() {
  ...
  if (error) state.authError = error.message || 'Sign out failed.';
...
async function _loadData() {
  ...
  state.rooms = rooms;
  state.filesByRoom = files.reduce((m, f) => {
    const a = m.get(f.room_id) || [];
    a.push(f);
    m.set(f.room_id, a);
    return m;
  }, new Map());
...
function _wireBackToSyncPad() {
  document.getElementById('admin-back-link')?.addEventListener('click', () => {});
}
function _wireErrorActions() {
  document.getElementById('admin-retry')?.addEventListener('click', () => {
    state.authError = '';
    state.adminCheckError = '';
    state.loading = false;
    _queueRender();
