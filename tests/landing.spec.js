...
import { goToLanding, roomIdFromUrl } from './helpers.js';
...
test.describe('Landing page', () => {
  ...
  test('"New room" creates a room and navigates to app screen', async ({ page }) => {
    await goToLanding(page);
  ...
  test('"New room" URL contains a valid room ID (no reserved paths)', async ({ page }) => {
    await goToLanding(page);
    await page.click('.landing-create-btn');
    await page.waitForSelector('#app-screen:not(.hidden)', { timeout: 15_000 });
  ...
  test('"Join room" input + button navigate to the typed room', async ({ page }) => {
    await goToLanding(page);
    await page.fill('.landing-join-input', 'test-join-room');
    await page.click('.landing-join-btn');
    await page.waitForSelector('#app-screen:not(.hidden)', { timeout: 15_000 });
    expect(page.url()).toContain('test-join-room');
  });

  test('"Join room" also works by pressing Enter in the input', async ({ page }) => {
    await goToLanding(page);
    await page.fill('.landing-join-input', 'enter-room-test');
    await page.press('.landing-join-input', 'Enter');
    await page.waitForSelector('#app-screen:not(.hidden)', { timeout: 15_000 });
    expect(page.url()).toContain('enter-room-test');
  });
